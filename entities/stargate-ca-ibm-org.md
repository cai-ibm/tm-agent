...
### Доступность сервера (20-21.06.2026)

- **20.06 08:00 UTC** — SSH таймаут (60с), сервер не отвечает по SSH
- **21.06 06:00–08:00 UTC** — SSH-соединение обрывается на этапе аутентификации (Connection timed out during banner exchange). Порт 22 открыт, хост пингуется (0.4ms). SSH-рукопожатие доходит до отправки ключа/пароля и зависает.
- **Вероятная причина:** зависший SSH-демон или fail2ban/rate-limit, заблокировавший IP отправителя
- **Статус:** мониторинг отключён пользователем 21.06 в 08:02 (cron paused)
