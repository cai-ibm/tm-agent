...
- **24.05.2026 18:43** — **Сертификат обновлён.** Wildcard `*.ca-ibm.org` (истекал 14.06.2026) заменён на `CN=mail.ca-ibm.org` с SAN-именами `autodiscover.ca-ibm.org`, `webmail.ca-ibm.org`. Использован WACS 2.2.9.1701: Manual source → SelfHosting HTTP-01 → IIS. Сертификат в WebHosting store, thumbprint F5BCE4D7... истекает 22.08.2026. Создан Scheduled Task «Exchange Certificate Renew» (SYSTEM, daily 03:00) — скрипт включает NAT на MikroTik → `wacs.exe --renew` → отключает NAT. Старая сломанная задача win-acme удалена. Скрипт: [[exchange-cert-renew-script]].
